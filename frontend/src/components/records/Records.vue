<template>
  <div class="max-w-md m-auto py-10">
    ...
    <form @submit.prevent="addRecord">
      ...
      <input type="submit" value="Add Record" class="btn green" />
    </form>
...
export default Vue.extend({
  ...
  async created() {
    if (!localStorage.signedIn) {
      ...
      try {
        const response = await this.$http.secured.get('/api/v1/records');
  ...
  methods: {
    ...
    async addRecord() {
      ...
      try {
        const response = await this.$http.secured.post('/api/v1/records/', {
          record: {
        ...
        this.setError(error, 'Cannot create record');
      }
    },
    removeRecord(record) {
      try {
        ...
        this.setError(error, 'Cannot delete record');
      }
    },
    editRecord(record) {
    ...
    updateRecord(record) {
      ...
      try {
        ...
        this.setError(error, 'Cannot update record');
      }
    },
  },
});
